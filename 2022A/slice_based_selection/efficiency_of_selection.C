...
void efficiency_of_selection(const bool save = true, TString saveDirExt = "basic", const std::vector<CutDef<Cut>> cuts = ncpizero_cuts, const bool test = false)
{
  selectionstyle();
  ...
  const TString saveDir = "/sbnd/data/users/hlay/ncpizero/2022A/plots/slice_based_selection/efficiency_of_selection/" + saveDirExt;
  ...
  for(auto const& plot : effPlots)
      ...
      for(auto const& cut : cuts)
	{
      ...
      if(save)
	{
	  canvas->SaveAs(saveDir + "/" + plot.label + "_eff.png");
	  canvas->SaveAs(saveDir + "/" + plot.label + "_eff.pdf");
	}
    }
